...
void sortMatrix(int** buff, int row, int col);
...
int inputStatic(int buff[LEN][LEN], int row, int col) {
    ...
    int newI = 0;
    int minSum = INT32_MAX, sum = 0, minIndex = 0;
    while (newI != row - 1) {
        for (int i = newI; i < row; i++) {
            for (int j = 0; j < col; j++) {
                sum += buff[i][j];
            }
            if (sum < minSum) {
        ...
        for (int j = 0; j < col; j++) {
            int temp = buff[newI][j];
            buff[newI][j] = buff[minIndex][j];
            buff[minIndex][j] = temp;
        }
        newI++;
    }
